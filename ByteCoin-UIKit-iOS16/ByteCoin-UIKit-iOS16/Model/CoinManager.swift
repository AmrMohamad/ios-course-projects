...
//
//  Created by Amr Mohamad on 04/05/2023.
//
...
struct CoinManager {
        ...
        if let url = URL(string: urlString){
            ...
            let task = session.dataTask(with: url) { (data:Data?, response:URLResponse?, error:Error?) in
                ...
                if let safeData = data {
                    if let priceData = parseJSON(data: safeData) {
    ...
    func parseJSON (data: Data) -> CoinModel? {
        ...
        do {
            ...
            let currencyName = decodedData.currency

            return CoinModel(priceExchangeRate: rate, currency: currencyName)
        } catch {
            print(error)
            return nil
        }
